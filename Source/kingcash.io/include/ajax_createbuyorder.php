<?php	
	$token = $_POST['token'];
    $kcpvolume = $_POST['createbuyorder_kcpvolume'];
    $btcpriceperkcp = $_POST['createbuyorder_btcpriceperkcp'];
    $btcvolume = $_POST['createbuyorder_btcvolume'];
    $google2facode = $_POST['createbuyorder_google2facode'];
    $btc_balance = $_POST['btc_balance'];
    $res['res'] = "true";
    if($btcvolume>$btc_balance){
        $res['res'] = "false";
        $res['msg'] = "Not enough BTC";
    }
    if($btcvolume<=0||$btcpriceperkcp<=0||$kcpvolume<=0){
        $res['res'] = "false";
    ...
    if($res['res']=="true"){
        ...
        $query = "INSERT INTO `tb_history` (
            ...
            'out',
            '".$btcvolume."',
            'Create buy order for KCP: ".number_format($kcpvolume, 8, '.', '')."',
            'btc',
            '1'
        )";
        if ($conn->query($query) === TRUE) {
    ...
    $query =  "SELECT * FROM `tb_orders` WHERE `f_ordertype`='sell' && `f_btcpriceperkcp`<='".$btcpriceperkcp."' ORDER BY `f_regdate` ASC";
    $result = $conn->query($query);
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            if (($this_order_data['f_kcpvolume']-$row['f_kcpvolume'])>=0.00000001){
                ...
                $query =  "SELECT `f_amount` FROM `tb_kcpwallet` WHERE `f_token`='".$this_order_data['f_token']."'";
                $temp_result = $conn->query($query);
                if ($temp_result->num_rows > 0) {
                    while($temp_row = $temp_result->fetch_assoc()) {
                ...
                $query = "UPDATE `tb_kcpwallet` SET `f_amount`='".$buyer_amount."' WHERE `f_token`='".$this_order_data['f_token']."'";
                $conn->query($query);  
                ...
                $seller_amount = $seller_amount + $row['f_btcvolume'];    
                $seller_amount = number_format($seller_amount, 8, '.', '');
                $query = "UPDATE `tb_btcwallet` SET `f_amount`='".$seller_amount."' WHERE `f_token`='".$row['f_token']."'";
                $conn->query($query); 
                $history_query =  "SELECT MAX(`f_id`) FROM `tb_history`";
                $history_regdate = date('Y-m-d H:i:s');
                $history_result = $conn->query($history_query);
                ...
                $conn->query($query); 
                $after_buy_kcpvolume = $this_order_data['f_kcpvolume']-$row['f_kcpvolume'];
                $query = "UPDATE `tb_orders` SET `f_kcpvolume`='".$after_buy_kcpvolume."' WHERE `f_id`='".$this_order_id."'";
                $conn->query($query);
                $after_buy_btcvolume = $this_order_data['f_btcvolume']-$row['f_kcpvolume']*$this_order_data['f_btcpriceperkcp']*(1+$exchange_sell_fee/100);
                $query = "UPDATE `tb_orders` SET `f_btcvolume`='".$after_buy_btcvolume."' WHERE `f_id`='".$this_order_id."'";
                $conn->query($query);
                ...
                $add_btcvolume = number_format($row['f_btcpriceperkcp']*$this_order_data['f_kcpvolume']*(1-$exchange_sell_fee/100), 8, '.', '');
